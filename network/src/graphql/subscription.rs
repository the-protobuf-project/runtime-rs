...
use super::named::{build_field_tag, build_variable_declarations, sorted_args, FieldArg};
use super::subscription_task::{await_connection_ack, run_subscription, send_json};
use super::ws_protocol::{ClientMessage, SubscribePayload, SUBPROTOCOL};
...
impl GraphQLClient {
    ...
    pub async fn subscribe_fields<T: DeserializeOwned + Send + 'static>(
        ...
        let mut request = full_url
            .as_str()
            .into_client_request()
            .map_err(|e| Error::SubscriptionStart(Box::new(Error::WsConnect { host: host.clone(), source: Box::new(e) })))?;
        request
            .headers_mut()
            .insert(http::header::SEC_WEBSOCKET_PROTOCOL, http::HeaderValue::from_static(SUBPROTOCOL));
        ...
        let (ws_stream, _resp) = tokio::time::timeout(self.options.timeout, dial)
            ...
            .map_err(|e| {
                Error::SubscriptionStart(Box::new(Error::WsConnect { host: host.clone(), source: Box::new(e) }))
            })?;
        ...
        let variables = if variables.is_empty() { None } else { Some(serde_json::Value::Object(variables)) };

        let sub_id = "1".to_string();
        send_json(
            &mut sink,
            &ClientMessage::Subscribe { id: sub_id.clone(), payload: SubscribePayload { query: &query, variables } },
        )
        .await
        .map_err(|e| Error::SubscriptionStart(Box::new(e)))?;
        ...
        Ok(Subscription { updates: rx, cancel, task, _marker: PhantomData })
    }
}
